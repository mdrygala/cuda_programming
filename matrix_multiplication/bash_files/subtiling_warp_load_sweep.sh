...
echo "----------------------------------------------------------"
echo "STARTING WARP LOAD PARAMETER SWEEP"
echo "----------------------------------------------------------"
    ...
    for TK in "${TILE_KS[@]}"; do
      for THREAD_DIM in "${THREAD_DIMS[@]}"; do
        for PAD in "${PADDINGS[@]}"; do
          ...
          CONFIG_STR="TILE_WARP_LOAD_M=$TM TILE_WARP_LOAD_N=$TN TILE_WARP_LOAD_K=$TK THREAD_DIM_WARP_LOAD=$THREAD_DIM PADDING_WARP_LOAD=$PAD THREADS=$THREADS"

          COMPILE_OUTPUT=$(nvcc -O3 -arch=sm_80 -lineinfo -Xptxas -v \
            -I. \
            -DTILE_WARP_LOAD_M=$TM \
            -DTILE_WARP_LOAD_N=$TN \
            -DTILE_WARP_LOAD_K=$TK \
            -DTHREAD_DIM_WARP_LOAD=$THREAD_DIM \
            -DPADDING_WARP_LOAD=$PAD \
            launch_subtiling_warp_load.cu \
            -o .build/warp_load_temp 2>&1)
...
/Used/ {
    if (current_kernel ~ /^void GEMMSubTilingLoadSlabGenDims/ || current_kernel ~ /^GEMMSubTilingLoadSlabGenDims/) {
        print "------------------------------------------------------------";
        print "CONFIG: " config;
...
'
          ...
          for DT in "${DATATYPES[@]}"; do
            if [ "$DT" = "float" ]; then
              OUTPUT=$(./.build/warp_load_temp 2>&1)
              RUN_LABEL="float"
            else
              OUTPUT=$(./.build/warp_load_temp --datatype half 2>&1)
              RUN_LABEL="half"
            fi
