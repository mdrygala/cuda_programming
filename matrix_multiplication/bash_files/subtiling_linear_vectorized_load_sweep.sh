...
echo "----------------------------------------------------------"
echo "STARTING LINEAR VECTORIZED LOAD PARAMETER SWEEP"
echo "----------------------------------------------------------"
    ...
    for TK in "${TILE_KS[@]}"; do
      for THREAD_DIM in "${THREAD_DIMS[@]}"; do
        for PAD in "${PADDINGS[@]}"; do
          ...
          # 16-byte vector constraints:
          # float uses 4 elements, half uses 8 elements.
          # Since this kernel supports both, use the stricter half constraint.
          if (( TK % 8 != 0 )); then continue; fi
          if (( TN % 8 != 0 )); then continue; fi

          CONFIG_STR="TILE_LINEAR_LOAD_M=$TM TILE_LINEAR_LOAD_N=$TN TILE_LINEAR_LOAD_K=$TK THREAD_DIM_LINEAR_LOAD=$THREAD_DIM PADDING_LINEAR_LOAD=$PAD THREADS=$THREADS"

          COMPILE_OUTPUT=$(nvcc -O3 -arch=sm_80 -lineinfo -Xptxas -v \
            -I. \
            -DTILE_LINEAR_LOAD_M=$TM \
            -DTILE_LINEAR_LOAD_N=$TN \
            -DTILE_LINEAR_LOAD_K=$TK \
            -DTHREAD_DIM_LINEAR_LOAD=$THREAD_DIM \
            -DPADDING_LINEAR_LOAD=$PAD \
            launch_subtiling_linear_vectorized_load.cu \
            -o .build/linear_vectorized_load_temp 2>&1)
...
/Used/ {
    if (current_kernel ~ /^void GEMMSubTilingLinearVectorized/ || current_kernel ~ /^GEMMSubTilingLinearVectorized/) {
        print "------------------------------------------------------------";
        print "CONFIG: " config;
...
'
          ...
          for DT in "${DATATYPES[@]}"; do
            if [ "$DT" = "float" ]; then
              OUTPUT=$(./.build/linear_vectorized_load_temp 2>&1)
              RUN_LABEL="float"
            else
              OUTPUT=$(./.build/linear_vectorized_load_temp --datatype half 2>&1)
              RUN_LABEL="half"
            fi
...
echo ""
echo "================ BEST LINEAR VECTORIZED LOAD CONFIGS ================"
...
rm -f .build/linear_vectorized_load_temp