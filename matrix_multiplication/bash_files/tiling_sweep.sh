...
echo "----------------------------------------------------------"
echo "STARTING TILING PARAMETER SWEEP"
echo "----------------------------------------------------------"
...
for TILE in "${TILES[@]}"; do
  ...
  CONFIG_STR="TILE=$TILE THREADS=$THREADS"

  COMPILE_OUTPUT=$(nvcc -O3 -arch=sm_80 -lineinfo -Xptxas -v \
    -I. \
    -DTILE=$TILE \
    launch_tiling.cu \
    -o .build/tiling_temp 2>&1)
...
/Used/ {
    if (current_kernel ~ /^void GEMMTiling/ || current_kernel ~ /^GEMMTiling/) {
        print "------------------------------------------------------------";
        print "CONFIG: " config;
...
'
  ...
  for DT in "${DATATYPES[@]}"; do
    if [ "$DT" = "float" ]; then
      OUTPUT=$(./.build/tiling_temp 2>&1)
      RUN_LABEL="float"
    else
      OUTPUT=$(./.build/tiling_temp --datatype half 2>&1)
      RUN_LABEL="half"
    fi
