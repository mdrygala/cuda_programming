...
void set_block_and_grid(dim3& block, dim3& grid, int M, int N){
    constexpr int THREADS_M = TILE_REGISTER_SCALAR / THREAD_DIM_REGISTER_SCALAR;
    constexpr int THREADS_N = TILE_REGISTER_SCALAR / THREAD_DIM_REGISTER_SCALAR;

    set_block_and_grid_subtile<
        TILE_REGISTER_SCALAR,
        TILE_REGISTER_SCALAR,
        THREADS_N,
        THREADS_M
    >(block, grid, M, N);
